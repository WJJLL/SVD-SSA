...
parser.add_argument('--model_name', type=str, default='resnet152', help='Output directory with adversarial images.')
...
parser.add_argument('--layer', type=str, default='layer4', help='layer of Source Models.')
parser.add_argument("--beta", type=float, default=0.5, help="Maximum size of adversarial perturbation.")
parser.add_argument("--topk", type=int, default=1, help="Number of iterations.")
...
def Spectrum_Simulation_Attack(images, gt, model, min, max,layer):
    """
    ...
    for i in range(num_iter):
        ...
        for n in range(N):
            ...
            _, output = SVD_model(model, x_idct, layer, opt.beta, opt.topk)
            # DI-FGSM
            # _, output = SVD_model(model, DI(x_idct), layer,opt.beta,opt.topk)
...
def main():
    ...
    if opt.model_name in ['inceptionv3','inceptionv4','inceptionresnetv2']:
    ...
    elif opt.model_name == 'inceptionv4':
        model = torch.nn.Sequential(Normalize(mean, std),
    ...
    for images, images_ID,  gt_cpu in tqdm(data_loader):
        ...
        adv_img = Spectrum_Simulation_Attack(images, gt, model, images_min, images_max,opt.layer)
        adv_img_np = adv_img.cpu().numpy()
        adv_img_np = np.transpose(adv_img_np, (0, 2, 3, 1)) * 255
        save_image(adv_img_np, images_ID, opt.output_dir+opt.model_name+'/')
