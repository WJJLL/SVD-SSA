...
parser.add_argument('--withSVD', action='store_false', default=True,
                    help='Use SVD')
...
def graph(model ,x, gt):
    ...
    for i in range(num_iter):
        ...
        grad = torch.mean(torch.stack(torch.chunk(torch.autograd.grad(loss, x_batch,retain_graph=False, create_graph=False)[0], 5),0)
